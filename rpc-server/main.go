...
var (
	redisClient = &RedisClient{} // make the RedisClient with global visibility in the 'main' scope
)
...
func main() {
	...
	err := redisClient.InitClient(ctx, "redis:6379", "")
	if err != nil {
		log.Fatalf("failed to init redis, err %v", err)
	}
